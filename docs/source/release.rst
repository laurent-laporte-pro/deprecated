...
and its changelog follows `Keep a Changelog <https://keepachangelog.com/en/1.0.0/>`_.
...
Add a new section at the top of :file:`CHANGELOG.rst` for the new version, marked as unreleased:
...
.. code-block:: sh
    ...
    git add src/deprecated/__init__.py python-deprecated.spec CHANGELOG.rst \
...
`GitHub CLI <https://cli.github.com/>`_ (use the date of the previous release):
...
- flag the breaking changes with a ``.. warning::`` directive at the top of the section;
...
Replace ``(unreleased)`` by the release date (ISO 8601 format) in :file:`CHANGELOG.rst`,
and adjust the underline length:
...
- Check the `PyPI page <https://pypi.org/project/Deprecated/>`_ and the
  `documentation <https://deprecated.readthedocs.io/en/latest/>`_ of the new version.
